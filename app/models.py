...
class Theme(db.Model):
    ...
    color1 = db.Column(db.String(7), nullable=False)  # hex, e.g. #ff0000
...
class Slider(db.Model):
    ...
    items = db.relationship("SliderItem", back_populates="slider", lazy=True)
...
class SliderItem(db.Model):
    ...
    def __repr__(self):
        return f"<SliderItem slider={self.slider_id} product={self.product_id} idx={self.order_index}>"
...
class Comment(db.Model):
    ...
    status = db.Column(db.String(20), default="oczekuje")  # oczekuje, zaakceptowany, odrzucony
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
